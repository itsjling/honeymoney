...
Requests constrain the response to model-eligible spending categories, with definitions and accounting-boundary guidance; they never include owners. When a response omits a row or gives an invalid item, Honeymoney retries only those rows once and keeps valid results. The status line names the configured model and shows which batch is in flight (`Categorizing via Ollama (qwen2.5:7b-instruct)... batch 2/20 (transactions 6-10 of 98, 4s)`). It ticks up every second while waiting, so a slow local model doesn't look stuck. If Ollama is unreachable, the model is missing, or a categorization is rejected, the import prints a warning explaining why and the affected rows stay uncategorized for interactive or manual review.
